...
export default function Explore () {   
    ...
    useEffect(() => {
        const fetchRecommendedSongs = async () => {
            ...
            try {
                ...
                // ARTISTA FAVORITO
                const { artist, songs } = await getRecommendedSongsBasedOnFavoriteArtist(currentUser.email);
                if (artist) {
                    setFavoriteArtist(artist);
                    setArtistRecommendedSongs(songs);
                }

                const { country, countryGenre, listSongs  } = await getRecommendedSongsBasedOnCountry(currentUser.email);
                if(country){
    ...
    return (
        <>
            ...
            <RecommendedSongsView category="genre" categoryName={favoriteGenre} songs={recommendedSongs} />
            <br />
            <RecommendedSongsView category="genre" categoryName={secondFavoriteGenre} songs={secondRecommendedSongs} />
            <br />
            <RecommendedSongsView category="artist" categoryName={favoriteArtist} songs={artistRecommendedSongs} />
            <br />                
